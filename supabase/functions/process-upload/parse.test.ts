  ...
  const detail = XLSX.utils.aoa_to_sheet(
    [
      ...
      [
        ...
        "THUY DOAN #3653",
        "21 Dental Group",
        "Vo, Ngoc Lan DDS - RTE: JES",
        new Date(Date.UTC(2026, 4, 13)),
        "Implant - Abutment CUSTOM Titanium",
        "In Production",
        0,
      ],
...
Deno.test(
  "parses a raw ZTECH export via column aliases (Client/Ordered/Products)",
  () => {
    const wb = XLSX.utils.book_new()
    const detail = XLSX.utils.aoa_to_sheet(
      [
...
Deno.test(
  "parses every data row of a detail sheet longer than the header scan window",
  () => {
    const wb = XLSX.utils.book_new()
    // Bulky non-detail sheet first, like the raw dumps that ride along in real exports.
    const raw = XLSX.utils.aoa_to_sheet([
      ["Dump", "Values"],
      ...Array.from({ length: 200 }, (_, i) => [i, `v${i}`]),
    ])
    ...
    const { rows, skipped } = parseWorkbook(bytes)
    assertEquals(skipped.length, 0)
    assertEquals(rows.length, 120)
    assertEquals(rows[119].patientName, "PATIENT 119")
    assertEquals(rows[119].orderDate, "2026-05-11")
  }
)

Deno.test(
  "returns the no-detail-sheet sentinel when no sheet has the expected header",
  () => {
    const wb = XLSX.utils.book_new()
    const pivot = XLSX.utils.aoa_to_sheet([
      ["Count of Product", "Column Labels"],
...
Deno.test(
  "names the missing columns when a sheet is close to the template",
  () => {
    const wb = XLSX.utils.book_new()
    ...
    assertStringIncludes(
      skipped[0].reason,
      'Closest sheet "Cases" is missing column(s): doctor, amount'
    )
  }
)
