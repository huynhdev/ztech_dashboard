-- ============================================================
-- Migration: get_dashboard_overview() — add redo counts
-- Purpose: Surface incoming_cases.is_redo as a `redo` count in the time series
--          (all three granularities) and the summary. Identical to
--          20260610040452_dashboard_overview_rpc.sql otherwise.
-- ============================================================
...
comment on function public.get_dashboard_overview(date, date) is
  'All aggregates for the dashboard overview page in one round trip (incl. redo counts); security invoker, so RLS on incoming_cases applies to the caller.';
