-- ============================================================
-- Migration: get_dashboard_overview() — add per-lab totalAmount to heatmapLabs
-- Purpose: The client heatmap shows a Total Cases column; surface the lab's
--          summed amount as `totalAmount` so a Total Amount column can be
--          rendered next to it. Identical to
--          20260611100002_dashboard_overview_redo.sql otherwise.
-- ============================================================
...
comment on function public.get_dashboard_overview(date, date) is
  'All aggregates for the dashboard overview page in one round trip (incl. redo counts and per-lab heatmap totalAmount); security invoker, so RLS on incoming_cases applies to the caller.';
