-- ============================================================
-- Migration: get_dashboard_overview() — add per-day amounts to heatmapLabs
-- Purpose: The client heatmap needs per-day amount sums per lab to render
--          TOTAL CASE/DAY and TOTAL AMOUNT/DAY footer rows. Adds an `amounts`
--          day→amount map alongside the existing `cells` day→count map.
--          Identical to 20260611100004_heatmap_redo_total.sql otherwise.
-- ============================================================
...
select jsonb_build_object(
  ...
  'heatmapLabs', coalesce((
    select jsonb_agg(jsonb_build_object(
      ...
      'amounts', amounts, 'totalAmount', total_amount, 'totalRedo', total_redo
    ))
    from (
...
comment on function public.get_dashboard_overview(date, date) is
  'All aggregates for the dashboard overview page in one round trip (incl. redo counts and per-lab heatmap day amounts/totalAmount/totalRedo); security invoker, so RLS on incoming_cases applies to the caller.';
